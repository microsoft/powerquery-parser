...
import { LexerState, UntouchedLine, LexerLineKind, LexerMultilineKind, LexStrategy } from "./lexerContracts";
...
export namespace Lexer {
    ...
    export function from(document: string): LexerState {
        return {
            lines: [lineFrom(document)],
            multilineKindUpdate: {},
        }
    }

    export function appendNewLine(state: LexerState, document: string, thenLex = true): LexerState {
        let newState: LexerState = {
            ...state,
            lines: state.lines.concat(lineFrom(document)),
        };

        if (thenLex) {
            newState = lexLine(newState);
        }

        return newState;
    }

    function lexLine(state: LexerState, lineNumber = (state.lines.length - 1)): LexerState {
        return state;
    }

    function lineFrom(document: string): UntouchedLine {
        return {
            kind: LexerLineKind.Untouched,
            multilineKind: LexerMultilineKind.Default,
            document,
            tokens: [],
            graphemes: StringHelpers.graphemeSplitter.splitGraphemes(document),
            actionsTaken: 0,
