...
function inspectScopeType(state: ScopeTypeInspectionState): ScopeTypeByKey {
    ...
    for (const scopeItem of scopeItemByKey.values()) {
        if (!state.givenTypeById.has(scopeItem.id)) {
            state.deltaTypeById.set(scopeItem.id, translateScopeItem(state, scopeItem));
        }
    }
...
function getOrCreateType(state: ScopeTypeInspectionState, xorNode: TXorNode): Type.TType {
    const nodeId: number = xorNode.node.id;

    const scope: ScopeItemByKey =

    const maybeGivenType: Type.TType | undefined = state.givenTypeMap.get(nodeId);
    if (maybeGivenType !== undefined) {
        return maybeGivenType;
    }

    const maybeDeltaType: Type.TType | undefined = state.givenTypeMap.if(state.givenTypeMap);
}
